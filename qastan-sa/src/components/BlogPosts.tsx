...
const Blog: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<string>(`${t('Blog.Cat')}`);
  ...
  // const filteredPosts = activeTab === `${t('Blog.Cat')}` ? blogPosts : blogPosts.filter(post => post.category === activeTab);
    ...
    <section className="container mx-auto my-20">
      ...
      <div className="gap-8 flex flex-wrap justify-center">
        {currentPosts.map((post, index) => {
          const translation = blogPostTranslations[post.id]?.[i18n.language];
          return (
            <div key={index} className="w-full relative h-96 lg:w-96 bg-gray-800 rounded-lg overflow-hidden shadow-lg">
                <img src={post.imageUrl} alt={translation?.title || post.title} className="w-full h-96 object-cover transition duration-500" />
                <div className="absolute inset-0 flex flex-col justify-end p-6">
                  <div className="bg-black bg-opacity-50 rounded-lg shadow-lg p-4">
                    <h3 className="text-white text-lg font-semibold mb-2">{translation?.title || post.title}</h3>
                    <div className='flex justify-between text-white font-semibold text-sm mb-0'>
                      <p className="text-white font-bold text-sm mb-0">{translation?.subtitle || post.subtitle}</p>
                      <div>
                        <Link to={`/Blog/${post.id}~${post.title.replace(' ', '-')}`} className='text-legoOrange flexborder-b border-legoOrange p-1'>{t('Blog.ReadMore')}</Link>
